use crate::models::user::{CreateUserError, DatabaseError};
...
pub enum HandlerError {
  ...
  ConvertIntegerToTimestamp(TryFromIntError),
  ...
  fn into_response(self) -> Response {
    match self {
      ...
      HandlerError::CreateUser(ref inner) => {
        error!("CreateUser: {}", inner);
        self.into_generic(StatusCode::INTERNAL_SERVER_ERROR)
      }
      HandlerError::ConvertIntegerToTimestamp(ref inner) => {
        error!("ConvertIntegerToTimestamp: {}", inner);
        self.into_generic(StatusCode::INTERNAL_SERVER_ERROR)
      }
      HandlerError::EncodeJwt(ref inner) => {
        error!("EncodeJwt: {}", inner);
        self.into_generic(StatusCode::INTERNAL_SERVER_ERROR)
      }
    }
    .into_response()
